...
describe('bridge extension manifest', () => {
  it('注册主进程、Scene 进程、工具管理与运行工作台窗口和最外层主菜单', () => {
    ...
    expect(nativeHost).toContain('style |= WsPopup | WsVisible | WsClipSiblings;');
    expect(nativeHost).toContain('exStyle |= WsExToolWindow;');
    expect(nativeHost).toContain('ClientToScreen(parentWindow, ref origin)');
    expect(nativeHost).toContain('StartBoundsTracking();');
    expect(nativeHost).toContain('Thread.Sleep(1);');
