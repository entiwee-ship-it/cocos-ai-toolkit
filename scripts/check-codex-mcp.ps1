...
if ($LASTEXITCODE -ne 0) {
  throw "Codex 未配置 cocos_ai: $config"
}
...
if ($configText -match '--enable-writes') {
  throw 'Codex cocos_ai 仍包含已移除的 --enable-writes 参数，请重新运行 install-codex-mcp.ps1'
}
...
if ($EndpointRoot) {
  $env:COCOS_AI_ENDPOINT_ROOT = [IO.Path]::GetFullPath($EndpointRoot)
}
